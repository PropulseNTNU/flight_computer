...
const int burnout_time = 7; // Milliseconds the rough estimate of how long the motor will burn
const int epsilon = 0; // a small time constatnt used for max and min deviation from burnout_time
...
int burnout_state(double data[]) {
	...
	if(burntime > (burnout_time + epsilon) && data[2] <= 0){
            ret_code = NEXT;
    }else{
            ret_code = REPEAT;
    }
